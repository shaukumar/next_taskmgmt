...
import { MdEdit, MdDelete, MdRestartAlt, MdNotifications } from 'react-icons/md';
...
const [notificationReceiver, setNotificationReceiver] = useState(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const taskRes = await fetch('https://node-task-management-api-1.onrender.com/api/tasks/', { headers });
        ...
        const userRes = await fetch('https://node-task-management-api-1.onrender.com/api/users/List', { headers });
  ...
  useEffect(() => {
    const applyFilters = () => {
      const filtered = userTasks.map(({ user, tasks }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`https://node-task-management-api-1.onrender.com/api/tasks/${taskToDelete}`, {
        method: 'DELETE',
        headers,
      });
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(`https://node-task-management-api-1.onrender.com/api/tasks/${taskToUpdate._id}`, {
        method: 'PUT',
        headers,
  ...
  return (
    <UserLayout>
      <div className="container py-4">
        <h2 className="mb-4" style={{textAlign:'center'}}>Tasks Assigned by You</h2>
        ...
        {filteredTasks.length === 0 ? (
          ...
          filteredTasks.map(({ user, tasks }) => (
            <div key={user._id} className="mb-5">
              <h4><strong>Assigned to:</strong> <span style={{ color: '#007BFF' }}>{user.name}</span></h4>
              <div className="row">
                {tasks.map((task) => (
                  <div key={task._id} className="col-md-4 mb-4">
                    <div className="card h-100 shadow-sm">
                      <div className="card-body">
                        ...
                        <p><strong>Task desc.:</strong> {task.description}</p>
                        <p><strong>Status:</strong> {task.status}</p>
                        <p><strong>Priority:</strong> {task.priority}</p>
                        <p><strong>Due date:</strong> <span className="text-muted">{new Date(task.due_date).toLocaleDateString()}</span></p>
                        <div className="d-flex justify-content-end gap-2">
                          ...
                          <button
    className="btn   btn-sm"style={{ borderColor: 'green', color: 'green' }}
    onClick={() => {
      setNotificationTask(task);
      setNotificationReceiver(task.assigned_to);
