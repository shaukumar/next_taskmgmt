...
import { MdOutlineAssignment, MdEdit, MdDelete, MdAddTask, MdRestartAlt} from "react-icons/md";
...
export default function CreateAndListTasks() {
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    // const matchesDueDate = filterDueDate ? dueDateFormatted === filterDueDate : true;
            ...
            <div className="col-md-4 mb-4" key={task._id}>
              <div className="card h-100">
                <div className="card-body d-flex flex-column">
                  <p className="card-title"><strong>Task title: </strong>{task.title}</p>
                  <p className="card-text mb-1"><strong>Project: </strong> {task.project_name}</p>
                  <p className="card-text mb-1"><strong>Assigned to: </strong> {task.assigned_to_name}</p>
                  ...
                  <p className="card-text mb-1"><strong>Due date: </strong> {new Date(task.due_date).toLocaleDateString()}</p>
                  <p className="card-text"><strong>Task desc.: </strong>{task.description}</p>
                  <div className="mt-auto d-flex justify-content-end gap-3">
  <button className="btn btn-sm btn-outline-primary d-flex align-items-center justify-content-center" onClick={() => editTask(task)}>
    <MdEdit size={18} />
  </button>
  <button className="btn btn-sm btn-outline-danger d-flex align-items-center justify-content-center" onClick={() => handleDelete(task._id)}>
    <MdDelete size={18} />
  </button>
</div>
                </div>
    ...
    <div className="col-12">
      <div className="alert alert-info text-center rounded d-flex flex-column align-items-center">
        <MdOutlineAssignment size={20} className=" text-secondary" />
        No tasks available.
      </div>
    </div>
  )}
